...
void Linear_trails(uint64 * trail_log, unsigned char total_rounds, unsigned char rounds , unsigned char * active_sbox_last2 , unsigned char mask_log[5][16] , FILE* trailfile );
...
void combination(unsigned char * temparray, unsigned char sbox_count, uint64 * trail_log, const unsigned char total_rounds, unsigned char rounds , unsigned char * active_sbox_last2 , unsigned char mask_log[10][16] , FILE* trailfile ){
	...
	{ //sbox count ==15
		...
		if(sboxmask == 0 ){
			...
			for(mask_log[rounds][15] = 1 ; mask_log[rounds][15] < 16 ; mask_log[rounds][15]++){
...
void Linear_trails(uint64 * trail_log, const unsigned char total_rounds, unsigned char rounds , unsigned char * active_sbox_last2 , unsigned char mask_log[10][16] , FILE* trailfile ){
...
while(1){
		trail_number = 0;
		overall_bias = 0;
		memset(mask_log,0xff,5*16);
		printf("How many rounds(2-10),otherwise quit: ");
		scanf("%d" , &rounds);
		if(rounds<2 || rounds>10)break;
